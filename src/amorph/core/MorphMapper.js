 ...
 * 
 * USAGE:
 * - Grid-View: morphMapper.getMorphType(field, value) → 'tag-morph'
 * - Detail-View: morphMapper.getMorphType(field, value, { prefix: 'detail-' }) → 'detail-tag-morph'
 * 
 * MORPH AVAILABILITY:
 * 
 * | Base Morph           | Grid-View (no prefix) | Detail-View (detail-) |
 * |---------------------|----------------------|----------------------|
 * | data-morph          | ✅ data-morph         | ✅ detail-data-morph  |
 * | tag-morph           | ✅ tag-morph          | ✅ detail-tag-morph   |
 * | text-morph          | ✅ text-morph         | ✅ detail-text-morph  |
 * | range-morph         | ✅ range-morph        | ✅ detail-range-morph |
 * | key-value-morph     | ✅ key-value-morph    | ⚠️ detail-keyvalue-morph (no hyphen!) |
 * | radar-chart-morph   | ✅ radar-chart-morph  | ✅ detail-radar-chart-morph |
 * | timeline-morph      | ✅ timeline-morph     | ✅ detail-timeline-morph |
 * | map-morph           | ✅ map-morph          | ✅ detail-map-morph   |
 * | boolean-morph       | ✅ boolean-morph      | ❌ uses text-morph    |
 * | number-morph        | ✅ number-morph       | ❌ uses text-morph    |
 * | name-morph          | ✅ name-morph         | ❌ uses text-morph    |
 * | image-morph         | ✅ image-morph        | ❌ uses text-morph    |
 * | list-morph          | ✅ list-morph         | ❌ uses data-morph    |
 * | progress-morph      | ✅ progress-morph     | ❌ uses range-morph   |
 * | bar-chart-morph     | ✅ bar-chart-morph    | ❌ uses data-morph    |
 * | pie-chart-morph     | ✅ pie-chart-morph    | ❌ uses data-morph    |
 * | sparkline-morph     | ✅ sparkline-morph    | ❌ uses data-morph    |
 */
...
const DETAIL_MORPHS_AVAILABLE = new Set([
  ...
  'timeline-morph',
  'map-morph'
  // Note: key-value-morph → detail-keyvalue-morph (handled specially)
]);
...
export class MorphMapper {
  ...
  getMorphType(fieldName, value, context = {}) {
    ...
    if (prefix === 'detail-') {
      // Special handling for key-value-morph → keyvalue-morph (no hyphen)
